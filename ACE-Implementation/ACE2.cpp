...
void printClusters(const vector<Node> &clusterHeads, const vector<Node> &nodes) {
    cout << "Clusters:" << endl;
    for (const auto &clusterHead : clusterHeads) {
        cout << "ClusterHead " << clusterHead.cluster_id << " (Node " << clusterHead.id << "):" << endl;
        cout << "  Followers:" << endl;
        for (const auto &follower : findFollowers(nodes, clusterHead.cluster_id)) {
            cout << "    Node " << follower.id << endl;
        }
    }
}
...
int main() {
    ...
    while (chrono::duration_cast<chrono::seconds>(chrono::high_resolution_clock::now() - start).count() < 70) {
        ...
        printClusters(clusterHeads, nodes);
